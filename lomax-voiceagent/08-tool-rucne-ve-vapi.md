...
| **Timeout** | `25` sekund |
| **Strict** (pokud formulář nabízí) | **zapni** — nástroj pak při vadném schématu hlásí viditelnou chybu v logu místo tichého prázdného `{}`. Uvidíš to jako *Schema validation error* v Calls → detail hovoru. |

> **Pokud po zapnutí Strict začnou i BĚŽNÉ hovory hlásit chybu schématu**, vypni to zpátky — je to diagnostický nástroj pro odhalení skutečné poruchy, ne trvalá povinnost.
...
> **Pozor u polí 5, 6 a 8:** povolené hodnoty jsou napsané rovnou v popisu, protože formulář obvykle nenabízí `enum`. Nemaž je — Make z nich dělá čitelné texty v e-mailu.
...
**6.** Name: `typ_produktu` · Type: **string** · Required: **ANO**
```
...
**7.** Name: `popis_zavady` · Type: **string** · Required: **ANO**
```
...
**8.** Name: `priorita` · Type: **string** · Required: **ANO**
```
...
**9.** Name: `bezpecnostni_riziko` · Type: **boolean** · Required: **ANO**
```
...
**10.** Name: `shrnuti_pro_technika` · Type: **string** · Required: **ANO**
```
Jedna až dvě věty pro technika: co se stalo a co si nejspíš bude potřebovat vzít s sebou.
```

**11.** Name: `telefon_jine` · Type: **string** · Required: ne
```
...
## KROK 3b — Zkontroluj Max Tokens u ASISTENTA (ne u nástroje)

Tohle je nejčastější příčina toho, že `arguments` chodí prázdné nebo useknuté, i když je schéma v pořádku. Nástroj má 22 polí a model musí celý JSON s odpovědí vygenerovat v jednom tahu — když dojde `maxTokens` dřív, než JSON dopíše, nejde ho naparsovat a VAPI pošle `{}`. VAPI to přiznává i ve vlastní dokumentaci: *„The default token limit is only 100. Increase it for complex tools."*

Nastav se to u **asistenta**, ne u tohoto nástroje — viz ČÁST 3 v `06-vapi-navod.md`, řádek **Max Tokens → `1500`**. Klidně to zkontroluj hned teď, ať na to nezapomeneš, až budeš pokračovat asistentem.
...
- [ ] **10 z nich** je označeno jako Required — jsou to prostě **čísla 1 až 10**, schválně jsou v novém pořadí na začátku
- [ ] `bezpecnostni_riziko` (9), `opakovana_zavada` (16) a `ma_fotografie` (21) jsou typu **boolean**, ne string
...
**Když nástroj zavolá, ale `arguments` je `{}` nebo mu chybí pole ke konci** (typicky `poznamka`, `ma_fotografie`, `dostupnost` — poslední v pořadí): to je oříznutí kvůli `maxTokens`, viz KROK 3b výš. Podrobný rozbor je v `11-oprava-prazdne-argumenty.md`.
